...
def prob_fix_color(original_circles, coordinates, colors, fig_size_h, fig_size_w,blur=1):
    ...
    dist_sum = torch.zeros([colors.shape[0],fig_size_h,fig_size_w]).to(coordinates.device)#构成一个全零的矩阵，尺寸是([颜色数量，图片高度，图片宽度])
    for color_idx in range(colors.shape[0]):
...
def ctrl_loss(circles, fig_h, fig_w, sigma=40):
    ...
    return loss_c
def ctrl_loss_vd(circles, fig_h, fig_w):
    sigma=40
    circles = circles.repeat(circles.shape[0], 1, 1)
    diff = circles - circles.permute(1, 0, 2)
    diff_ell2 = (diff[..., 0] * diff[..., 0] * fig_h * fig_h + diff[..., 1] * diff[..., 1] * fig_w * fig_w)
    loss_c = torch.exp(-diff_ell2 / (sigma ** 2)).mean() - 1 / circles.shape[1]
    return loss_c
def voronoi_diagram(point_number,coordinates,fig_size_h, fig_size_w,color_list,device):
    mask1=torch.ones_like(color_list).to(device)*255.0
    mask0 = torch.zeros_like(color_list).to(device)
    mean=torch.mean(color_list)
    color=torch.where(color_list>mean,mask1,mask0).repeat(1,3)
