...
class PatchTrainer(object):
    def __init__(self, args):
        ...
        if args.arch == "rcnn":
            ...
            self.model = torch.hub.load('facebookresearch/detr:main', 'detr_resnet50', pretrained=True).eval().to(device)
        elif args.arch == "deformable-detr":
            self.model = DeformableDetrForObjectDetection.from_pretrained("/data0/BigPlatform/tdy/Adversarial_camou-main/deformable-detr").eval().to(device)
    ...
    def update_mesh(self, tau=0.3, type='gumbel'):
        # camouflage:
        # tex=voronoi_diagram(self.tshirt_point_num,self.coordinates,self.h, self.w,self.color_list,self.device).unsqueeze(0)
        # tex_trouser=voronoi_diagram(self.tshirt_point_num,self.coordinates,self.h_t, self.w_t,self.color_list_t,self.device).unsqueeze(0)
        ...
        gb_tshirt = -(-(self.seeds_tshirt_train + 1e-20).log() + 1e-20).log()
        gb_trouser = -(-(self.seeds_trouser_train + 1e-20).log() + 1e-20).log()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', default='cuda:6', help='')
    ...
    parser.add_argument("--num_points_tshirt", type=int, default=50, help='')
    parser.add_argument("--num_points_trouser", type=int, default=50, help='')
