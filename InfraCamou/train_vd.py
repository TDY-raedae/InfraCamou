...
class PatchTrainer(object):
    def __init__(self, args):
        ...
        if args.arch == "rcnn":
            ...
            self.model = DeformableDetrForObjectDetection.from_pretrained(
                "/data0/BigPlatform/tdy/Adversarial_camou-main/deformable-detr").eval().to(device)
        ...
        self.batch_size = args.batch_size  # 定义批大小

        self.patch_transformer = PatchTransformer().to(device)  # 获取PatchTransformer，做EOT
        # 获取模型对应的解码器
        if args.arch == "rcnn":
        ...
        resolution = 4
        h, w, h_t, w_t = int(self.fig_size_H / resolution), int(self.fig_size_W / resolution), int(
            self.fig_size_H_t / resolution), int(self.fig_size_W_t / resolution)
        ...
        self.tshirt_point_num = torch.rand([self.tshirt_point.shape[1], 2], requires_grad=True, device=device)
        self.trouser_point = torch.rand([num_colors, args.num_points_trouser, 3], requires_grad=True, device=device)
        self.trouser_point_num = torch.rand([self.trouser_point.shape[1], 2], requires_grad=True, device=device)
        self.color_list = torch.rand([args.num_points_tshirt, 1], requires_grad=True, device=device)
        self.color_list_t = torch.rand([args.num_points_trouser, 1], requires_grad=True, device=device)
        # self.colors = torch.load("data/camouflage4.pth").float().to(device)
        self.colors = torch.tensor([[0, 0, 0], [1, 1, 1]]).float().to(device)
        # 加载为网格
        ...
        self.optimizer = torch.optim.Adam([self.tshirt_point_num, self.trouser_point_num], lr=args.lr)

        self.optimizer_seed = torch.optim.Adam([self.color_list, self.color_list_t], lr=args.lr_seed)
    ...
    def initialize_tps2d(self):
        locations_tshirt_ori = torch.load(
            os.path.join(self.DATA_DIR, 'Archive/tshirt_join/projections/part_all_2p5.pt'), map_location='cpu').to(
            self.device)
        self.infos_tshirt = MU.get_map_kernel(locations_tshirt_ori, self.faces_uvs_tshirt)

        locations_trouser_ori = torch.load(
            os.path.join(self.DATA_DIR, 'Archive/trouser_join/projections/part_all_off3p4.pt'), map_location='cpu').to(
            self.device)
    ...
    def synthesis_image(self, img_batch, use_tps2d=True, use_tps3d=True):
        ...
        if use_tps2d:
            # tps_2d
            source_control_points_tshirt = p3dmd.get_points(self.tshirt_locations_infos,
                                                            torch.pi / 180 * args.tps2d_range_t, args.tps2d_range_r,
                                                            bs=self.batch_size, random=True)
            locations_tshirt = self.tps2d_tshirt(source_control_points_tshirt.to(self.device))
            source_control_points_trouser = p3dmd.get_points(self.trouser_locations_infos,
                                                             torch.pi / 180 * args.tps2d_range_t, args.tps2d_range_r,
                                                             bs=self.batch_size, random=True)
        ...
        cnt = 0
        for img in p_img_batch:
            fig = plt.figure()
            plt.imshow(img.permute(1, 2, 0).detach().cpu().numpy())
            plt.axis('off')
            fig.savefig("./mid_image/" + str(cnt) + ".png")
            cnt += 1
        return p_img_batch, gt

    def update_mesh(self, tau=0.3, type='gumbel'):
        # camouflage:
        tex = voronoi_diagram(self.tshirt_point_num, self.coordinates, self.h, self.w, self.color_list,
                              self.device).unsqueeze(0)
        tex_trouser = voronoi_diagram(self.tshirt_point_num, self.coordinates, self.h_t, self.w_t, self.color_list_t,
                                      self.device).unsqueeze(0)
        ...
        self.mesh_trouser.textures = TexturesUV(maps=tex_trouser, faces_uvs=self.faces_trouser,
                                                verts_uvs=self.verts_uv_trouser)
    ...
    def train(self):
        ...
        for epoch in tqdm(range(checkpoints, args.nepoch)):
            ...
            if epoch % 100 == 99:
                ...
                self.optimizer_seed.param_groups[0]['lr'] = self.optimizer_seed.param_groups[0][
                                                                'lr'] / args.lr_decay_seed
            ...
            for i_batch, img_batch in enumerate(self.train_loader):
                img_batch = torch.zeros_like(img_batch.to(self.device))
                t0 = time.time()
                # AG step
                ...
                t2 = time.time()
                try:
                    det_loss, max_prob_list = self.prob_extractor(output, gt, loss_type=args.loss_type,
                ...
                loss_c = ctrl_loss_vd(self.tshirt_point_num, self.fig_size_H, self.fig_size_W)
                loss_c += ctrl_loss_vd(self.trouser_point_num, self.fig_size_H_t, self.fig_size_W_t)
                loss += args.ctrl * loss_c
                # print("args.cdist:",args.cdist)
                # if args.cdist != 0:
            ...
            if (epoch + 1) % 100 == 0 or epoch == 0:
                ...
                plt.axis('off')
                fig.savefig("./image/maps_tshirt_test" + str(epoch) + "png")
                self.writer.add_figure('maps_tshirt', fig, epoch)

                fig = plt.figure()
                plt.imshow(tex_trouser[0].detach().cpu().numpy())
                plt.axis('off')
                fig.savefig("./image/maps_trouser_test" + str(epoch) + "png")
            ...
            if (epoch + 1) % 50 == 0:
                ...
                torch.save(self.tshirt_point_num, path)
                ...
                torch.save(self.trouser_point_num, path)
                ...
                # np.savez(path, loss_history=self.loss_history.cpu().numpy(), num_history=self.num_history.cpu().numpy(),
                #          azim=self.azim.cpu().numpy())

            if (epoch + 1) % 300 == 0:
                self.update_mesh(type='determinate')
                for iou_thresh in [0.01, 0.1, 0.3, 0.5]:
                    precision, recall, avg, confs, thetas = self.test(conf_thresh=0.01, iou_thresh=iou_thresh,
                                                                      angle_sample=37,
    ...
    def test(self, conf_thresh, iou_thresh, num_of_samples=100, angle_sample=37, use_tps2d=True, use_tps3d=True,
             mode='person'):
        """
        Optimize a patch to generate an adversarial example.
        :return: Nothing
        ...
        with torch.no_grad():
            ...
            for i_batch, img_batch in tqdm(enumerate(self.test_loader), total=len(self.test_loader), position=0):
                ...
                for it, theta in enumerate(thetas_list):
                    ...
                    output = self.model(p_img_batch)
                    total += len(
                        p_img_batch)  # since 1 image only has 1 gt, so the total # gt is just = the total # images
                    pos = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', default='cuda:4', help='')
    ...
    parser.add_argument("--num_points_tshirt", type=int, default=750, help='')
    parser.add_argument("--num_points_trouser", type=int, default=750, help='')
    ...
    parser.add_argument("--seed_ratio", default=1.0, type=float,
                        help='The ratio of trainable part when seed type is variable')
    ...
    if not args.test:
        ...
        precision, recall, avg, confs, thetas = trainer.test(conf_thresh=0.01, iou_thresh=args.test_iou,
                                                             angle_sample=37, use_tps2d=not args.disable_test_tps2d,
                                                             use_tps3d=not args.disable_test_tps3d, mode=args.test_mode)
        info = [precision, recall, avg, confs]
        path = args.save_path + '/' + str(epoch) + 'test_results_tps'
